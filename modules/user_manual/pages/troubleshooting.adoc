...
This section contains a list of error messages that ownCloud displays when things go wrong.
For each message, you can find the most likely cause, a resolution, and one or more translations.
...
During file uploads the file data is read from PHP input and copied into a part file on the target storage.

If the target storage is not local (eg: FTP) and that storage is slow, not available, or broken it is likely that the operation will fail either at the beginning, or in the middle of the copy.
Other reasons for this message can be that, when writing to external storage, the connection took too long to respond or the network connection was flaky.
...
It will always tell you that it cannot login on your server.
